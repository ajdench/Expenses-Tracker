...
window.UI.renderTrips = async function(selectedTripId = null) {
  const app = document.getElementById('app');
  if (!app) return;
  window.App.dbg('renderTrips'); // Use window.App.dbg
  await window.UI.renderShell(); // Call the extracted renderShell
  await window.UI.renderTripLists(selectedTripId); // Call the extracted renderTripLists
}

window.UI.renderShell = async function() {
  const app = document.getElementById('app');
  if (!app) return;
  window.App.dbg('renderShell'); // Use window.App.dbg
  const icons = await window.UI.loadIconSettings(); // Use window.UI.loadIconSettings
  const scan = await window.UI.loadScanSettings(); // Use window.UI.loadScanSettings
  app.innerHTML = `
    <div class="container">
  ...
  `;
  document.getElementById('settings-btn').addEventListener('click', window.UI.renderSettingsPage); // Call window.UI.renderSettingsPage
  document.getElementById('archive-trips-open')?.addEventListener('click', window.UI.renderArchivedTrips); // Call window.UI.renderArchivedTrips
  document.getElementById('trip-archive-drop')?.addEventListener('click', window.UI.renderArchivedTrips); // Call window.UI.renderArchivedTrips
}

// ... (The rest of the file content up to buildExpenseCard)

window.UI.buildTripCard = function(trip, isSelected) {
